...
const firebaseConfig = {
  apiKey: "AIzaSyAz0sTQXL6XDaYTNtsskhjzmEXEYSa4P3Y",
  authDomain: "dreamdecoder-af2e6.firebaseapp.com",
  projectId: "dreamdecoder-af2e6",
  storageBucket: "dreamdecoder-af2e6.firebasestorage.app",
  messagingSenderId: "539904149500",
  appId: "1:539904149500:web:6783ae436d9e130ee01479"
};
...
const T = {
  en: {
    ...
    verifyTitle:"Verify Your Email 📧",verifyMsg:"Verification link sent to:",
    verifyCheck:"✅ I verified — Continue",verifyResend:"Resend Email",verifyBack:"← Back",
    notVerified:"❌ Not verified yet. Check Gmail and click the link.",
    emailExists:"❌ Email already registered. Please login.",usernameExists:"❌ Username taken.",
    noAccount:"❌ No account found. Please register.",wrongPass:"❌ Incorrect password.",
    invalidEmail:"❌ Email must end with @gmail.com",shortPass:"❌ Min. 6 characters.",
    emptyField:"❌ Fill in all fields.",networkErr:"❌ Network error.",
    accountActive:"✅ Welcome back!",registered:"✅ Verification email sent to Gmail!",
    codeSent:"✅ Email resent!",analyzesLeft:"analyzes left",plan:"Plan",
    ...
    allViews:"🌐 All",islamic:"☪️ Islamic",psych:"🧠 Psych",biblical:"✝️ Biblical",advice:"💡 Advice",
    copy:"📋 Copy",copied:"✓ Copied!",whatsapp:"💬 WhatsApp",telegram:"✈️ Telegram",print:"📄 PDF",
    searchDreams:"🔍 Search...",noDreams:"No dreams yet. Tap + to start!",noMatch:"No results.",
    ...
    insightLocked:"🔒 Advanced Insights",insightLockedDesc:"Upgrade to Pro or Elite to unlock personal AI dream analysis.",
    upgradePlan:"⭐ Upgrade Plan",
    settingsTitle:"Settings",accountTab:"Account",planTab:"Plan",themeTab:"Theme",langTab:"Language",aboutTab:"About",
    ...
    buyContact:"Contact on Telegram to buy a plan. You'll receive a unique promo code.",
    ...
    enableNotif:"🔔 Enable Morning Reminders",notifEnabled:"✅ Reminders enabled!",notifDenied:"❌ Allow notifications in settings.",
  },
  ru: {
    ...
    verifyTitle:"Подтвердите Email 📧",verifyMsg:"Письмо отправлено на:",
    verifyCheck:"✅ Подтвердил — Продолжить",verifyResend:"Отправить снова",verifyBack:"← Назад",
    notVerified:"❌ Не подтверждён. Проверьте Gmail.",
    emailExists:"❌ Email уже зарегистрирован.",usernameExists:"❌ Имя занято.",
    noAccount:"❌ Аккаунт не найден.",wrongPass:"❌ Неверный пароль.",
    invalidEmail:"❌ Email должен быть @gmail.com",shortPass:"❌ Мин. 6 символов.",
    emptyField:"❌ Заполните все поля.",networkErr:"❌ Ошибка сети.",
    accountActive:"✅ Добро пожаловать!",registered:"✅ Письмо отправлено на Gmail!",
    codeSent:"✅ Письмо отправлено повторно!",analyzesLeft:"анализов",plan:"Тариф",
    ...
    settingsTitle:"Настройки",accountTab:"Аккаунт",planTab:"Тариф",themeTab:"Тема",langTab:"Язык",aboutTab:"О нас",
    ...
    enableNotif:"🔔 Включить напоминания",notifEnabled:"✅ Включено!",notifDenied:"❌ Разрешите уведомления.",
  },
  uz: {
    ...
    verifyTitle:"Emailni tasdiqlang 📧",verifyMsg:"Tasdiqlash xati yuborildi:",
    verifyCheck:"✅ Tasdiqladim — Davom etish",verifyResend:"Qayta yuborish",verifyBack:"← Orqaga",
    notVerified:"❌ Hali tasdiqlanmagan. Gmailni tekshiring.",
    emailExists:"❌ Email band. Kiring.",usernameExists:"❌ Ism band.",
    noAccount:"❌ Hisob topilmadi.",wrongPass:"❌ Noto'g'ri parol.",
    ...
    accountActive:"✅ Xush kelibsiz!",registered:"✅ Tasdiqlash xati yuborildi!",
    codeSent:"✅ Xat qayta yuborildi!",analyzesLeft:"tahlil qoldi",plan:"Tarif",
    ...
    settingsTitle:"Sozlamalar",accountTab:"Hisob",planTab:"Tarif",themeTab:"Mavzu",langTab:"Til",aboutTab:"Haqida",
    ...
    enableNotif:"🔔 Ertalabki eslatmalar",notifEnabled:"✅ Yoqildi!",notifDenied:"❌ Ruxsat bering.",
  }
};
...
async function fsPatch(col, id, data) {
  ...
  catch(e) { console.error("fsPatch",e); return false; }
}
async function fsQueryOne(col, field, value) {
  try {
    const q = query(collection(db,col),where(field,"==",value));
    const s = await getDocs(q);
    return s.empty ? null : s.docs[0].data();
  } catch(e) { console.error("fsQuery",e); return null; }
}
...
function calcStreak(dreams){
  ...
  for(const day of days){const d=new Date(day);d.setHours(0,0,0,0);if(Math.round((cur-d)/86400000)<=1){streak++;cur=d;}else break;}
  return streak;
}
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    fsGet("users",session.uid).then(async d=>{
      if(d){
        setUsageRaw({count:Number(d.count)||0,analyzes:Number(d.analyzes)||FREE_LIMIT,plan:d.plan||"free",subLogs:d.subLogs||[]});
        setDreams(d.dreams||[]);
      } else {
        await fsPatch("users",session.uid,{count:0,analyzes:FREE_LIMIT,plan:"free",subLogs:[],dreams:[],email:session.email,username:session.username});
      }
      setLoaded(true);
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"20px"}}>
          {[["📜",t.history,"history"],["📊",t.insights,"insights"],["🔍",t.dictionary,"dictionary"],["⚙️",t.settings,"settings"]].map(([ic,lb,sc])=>(
            <button key={sc} onClick={()=>nav(sc)} style={{background:cardBg,border:`1px solid ${borderCol}`,borderRadius:"14px",padding:"14px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"flex-start",gap:"5px",outline:"none"}}>
              <span style={{fontSize:"22px"}}>{ic}</span><span style={{fontSize:"13px",color:textCol,fontWeight:"500"}}>{lb}</span>
            </button>
          ))}
        </div>
        {favs.length>0&&(
...
function AuthScreen({S,C,t,lang,setLang,T,setSession,isDark,textCol,subCol,borderCol}){
  ...
  async function handleGoogle(){
    setBusy(true);setMsg("");
    try{
      const authUrl=`https://accounts.google.com/o/oauth2/v2/auth?client_id=728040509147-q617oh6ung4m7qtgbsntqfr98uvu3mos.apps.googleusercontent.com&redirect_uri=${encodeURIComponent(window.location.origin)}&response_type=token&scope=email%20profile&prompt=select_account`;
      const popup=window.open(authUrl,"googleSignIn","width=500,height=600,scrollbars=yes");
      if(!popup){err("❌ Popup blocked. Allow popups.");return;}
      const timer=setInterval(async()=>{
        try{
          ...
          if(url.includes("access_token")){
            ...
            const existing=await fsGet("accounts",uid);
            if(!existing) await fsPatch("accounts",uid,{uid,email:gEmail.toLowerCase(),username:gName,username_lower:gName.toLowerCase(),createdAt:new Date().toISOString(),verified:true,provider:"google"});
            const userData=await fsGet("users",uid);
            if(!userData) await fsPatch("users",uid,{count:0,analyzes:FREE_LIMIT,plan:"free",subLogs:[],dreams:[],email:gEmail,username:existing?.username||gName});
            succ(t.accountActive);
            setTimeout(()=>setSession({uid,email:gEmail,username:existing?.username||gName,idToken:"google_"+Date.now()}),600);
          }
        }catch{}
  ...
  async function handleLogin(){
    ...
    try{
      ...
      if(e.code==="auth/user-not-found"||e.code==="auth/invalid-credential"||e.code==="auth/invalid-email") return err(t.noAccount);
      if(e.code==="auth/wrong-password") return err(t.wrongPass);
      if(e.code==="auth/too-many-requests") return err("❌ Too many attempts. Try later.");
      err("❌ "+e.message);
    }
  }
  ...
  async function handleForgotPassword(){
    if(!resetEmail.trim()) return err(t.emptyField);
    if(!/^[^@]+@gmail\.com$/i.test(resetEmail)) return err(t.invalidEmail);
    setBusy(true);setMsg("");
    try{
      await sendPasswordResetEmail(auth,resetEmail);
      succ("✅ Password reset link sent to your Gmail!");
      setTimeout(()=>{setResetStep(false);setMode("login");setMsg("");},2000);
    }catch(e){err("❌ "+e.message);}
    setBusy(false);
  }
  ...
  if(resetStep) return (
    <div style={S.app}>
      ...
      <div style={S.body}>
        <button style={S.backBtn} onClick={()=>{setResetStep(false);setMsg("");}}>{t.back}</button>
        ...
        <input style={S.inp} placeholder="your@gmail.com" value={resetEmail} onChange={e=>setResetEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleForgotPassword()}/>
        {msg&&<div style={S.msg(ok)}>{msg}</div>}
        <button style={S.gradBtn} onClick={handleForgotPassword} disabled={busy}>{busy?"⏳...":"📧 Send Reset Link"}</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.body}>
        ...
        {mode==="login"&&<button onClick={()=>{setResetStep(true);setResetEmail(form.email);setMsg("");}} style={{background:"none",border:"none",color:C.gold,cursor:"pointer",fontFamily:"inherit",fontSize:"13px",padding:"0 0 10px",textDecoration:"underline"}}>{t.forgotPass}</button>}
        {msg&&<div style={S.msg(ok)}>{msg}</div>}
...
// ── PAYWALL ────────────────────────────────────────────────────
function Paywall({S,C,t,usage,setUsage,setScreen,goBack,isDark,textCol,subCol,borderCol,PLANS,session}){
  const [promo,setPromo]=useState("");const [msg,setMsg]=useState("");const [ok,setOk]=useState(false);const [busy,setBusy]=useState(false);
  async function apply(){
    const code=promo.trim().toUpperCase();
    if(!code) return;
    setBusy(true);setMsg("");
    try{
      const promoDoc=await fsGet("promoCodes",code);
      if(!promoDoc||promoDoc.used){setMsg(t.invalidPromo);setOk(false);setBusy(false);return;}
      const analyzes=Number(promoDoc.analyzes)||25;
      await fsPatch("promoCodes",code,{used:true,usedBy:session?.email||"unknown",usedAt:new Date().toISOString()});
      addUsedCode(code);
      const log={date:new Date().toISOString(),plan:promoDoc.plan,analyzes,code};
      setUsage(u=>({...u,analyzes:(u.analyzes-u.count)+analyzes,count:0,plan:promoDoc.plan,subLogs:[log,...(u.subLogs||[])]}));
      ...
      <div style={S.body}>
        ...
        <div style={S.card}>
          ...
          <div style={{display:"flex",gap:"8px",marginBottom:"6px"}}>
            ...
            <button onClick={apply} disabled={busy} style={{background:C.purple,border:"none",borderRadius:"10px",padding:"0 16px",color:"#fff",cursor:"pointer",fontFamily:"inherit",fontSize:"13px",fontWeight:"bold",whiteSpace:"nowrap"}}>{busy?"⏳":t.applyBtn}</button>
          </div>
...
function Settings({S,C,t,T,session,setSession,theme,setTheme,lang,setLang,isDark,textCol,subCol,borderCol,usage,setUsage,PLANS,goBack,dreams}){
  ...
  async function applyPromo(){
    ...
    try{
      const promoDoc=await fsGet("promoCodes",code);
      if(!promoDoc||promoDoc.used){setPmsg(t.invalidPromo);setPok(false);return;}
      const analyzes=Number(promoDoc.analyzes)||25;
      await fsPatch("promoCodes",code,{used:true,usedBy:session?.email||"unknown",usedAt:new Date().toISOString()});
      addUsedCode(code);
      const log={date:new Date().toISOString(),plan:promoDoc.plan,analyzes,code};
  ...
  async function enableNotifications(){
    ...
    if(perm==="granted"){
      ...
      setTimeout(()=>new Notification("🌙 DreamDecoder",{body:"Did you have a dream last night? Record it before you forget!"}),next-now);
